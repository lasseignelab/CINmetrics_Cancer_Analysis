...
```{r}
## This function will take the all.cinmetrics.df, subset the tumor samples, and will plot a heatmap for all of the CINmetrics across all 33 cancers.

get_cinmetrics_heatmap <- function(all.cinmetrics.df){
...
spearman.tumor.cinmetrics <- tumor.cinmetrics.df[,c(5:10)] %>% drop_na() %>% cor(., method = "spearman")
...
tumor.cinmetrics.heatmap <- Heatmap(spearman.tumor.cinmetrics,
                                    ...
                                    column_names_rot = 45,
                                    heatmap_legend_param = list(title = "Value", title_gp = gpar(fontsize = 10, fontface = "bold")))
return(tumor.cinmetrics.heatmap)
}

tumor.cinmetrics.heatmap <- get_cinmetrics_heatmap(all.cinmetrics.df)
tumor.cinmetrics.heatmap
```

```{r}
## This function will subset the tumor samples from the all.cinmetrics.df and plot a heatmap for all of the CINmetrics separated by each of the 33 cancers. 

get_cinmetrics_cancer_heatmap <- function(all.cinmetrics.df){}
tumor.cinmetrics.df <- all.cinmetrics.df %>% filter(sample_type %in% c("Metastatic", "Primary Blood Derived Cancer", "Primary Tumor", "Recurrent Tumor", "Additional - New Primary", "Primary Blood Derived Cancer - Peripheral Blood"))
