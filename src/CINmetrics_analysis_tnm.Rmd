...
metastasis.utest.df <- read_csv("data/processed/metastasis_utest_df.csv")
node.utest.df <- read_csv("data/processed/node_utest_df.csv")
tumor.stage.utest.df <- read_csv("data/processed/tumor_stage_utest_df.csv")
...
get_utest_graphs <- function(tnm.category, cancer.project){
  if(tnm.category == "tumor.stage"){
    # Filter for T1 and T4 and -Log10 transformation of p.adj values
    ...
    log.tumor.stage.utest <- filtered.tumor.stage.utest %>% select(p.adj) %>% sapply(., neglog10)
    ...
    base.segments.barplot <- ggbarplot(base.segments.utest,
                                      ...
                                      font.ytickslab = c(5, "bold"),) + geom_hline(yintercept = 1.30103, linetype = "dashed", color = "red")
    # Breakpoints Plot
    ...
    break.points.barplot <- ggbarplot(break.points.utest,
                                       ...
                                       font.y = c(10, "bold"),
                                       font.xtickslab = c(10, "bold"),
                                      font.ytickslab = c(7, "bold"),) + geom_hline(yintercept = 1.30103, linetype = "dashed", color = "red")
    # TAI bar plot
    ...
    tumor.stage.barplot <- ggarrange(base.segments.barplot,
  ...
  if(tnm.category == "metastasis"){
    ...
    # Tumor Stage Bar Plot
    metastasis.barplot <- ggarrange(base.segments.barplot,
                                     break.points.barplot,
                                     tai.barplot,
                                     fga.barplot,
                                     cna.barplot,
