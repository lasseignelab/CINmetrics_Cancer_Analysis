...
```{r}
## We create vectors with the 22 cancer names to loop through 
...
cancer.project <- c("TCGA-KIRC", "TCGA-BRCA", "TCGA-MESO", "TCGA-READ", "TCGA-KIRP", "TCGA-PAAD", "TCGA-CESC", "TCGA-ESCA", "TCGA-KICH", "TCGA-UVM", "TCGA-SKCM", "TCGA-COAD", "TCGA-LUSC", "TCGA-HNSC", "TCGA-TGCT", "TCGA-THCA", "TCGA-LIHC", "TCGA-BLCA", "TCGA-CHOL", "TCGA-STAD", "TCGA-LUAD")


metrics <- c("tai", "modified_tai", "cna","base_segments", "break_points", "fga")

## This function takes the all.tnm.df and performs a Mann-Whitney U-test on each of the stages depending on TNM 
get_utest_data <- function(all.tnm.df, cancer.project, tnm.category, metrics){
  if(tnm.category == "tumor.stage"){
    ...
    tumor.stage.utest <- data.frame(.y. = character(),
                        group1 = character(),
                        group2 = character(),
                        n1 = numeric(),
                        n2 = numeric(),
    ...
    tumor.stage.df <- subset(all.tnm.df, project == i)
    # Filtering the dataframe to only have the tumor stages and tumor samples we want included in the analysis
    ...
    utest.results$project <- i
    utest.results$cin <- j
    tumor.stage.utest <- rbind(tumor.stage.utest, utest.results)
    }
    }
    tumor.stage.utest <- tumor.stage.utest %>% select(cin, project, group1, group2, n1, n2, statistic, p, p.adj, p.adj.signif)
    tumor.stage.utest$group1 <- ifelse(tumor.stage.utest$group1 %in% "1", "T1", ifelse(tumor.stage.utest$group1 %in% "2", "T2", ifelse(tumor.stage.utest$group1 %in% "3", "T3", ifelse(tumor.stage.utest$group1 %in% "4", "T4", tumor.stage.utest$group1))))
    tumor.stage.utest$group2 <- ifelse(tumor.stage.utest$group2 %in% "1", "T1", ifelse(tumor.stage.utest$group2 %in% "2", "T2", ifelse(tumor.stage.utest$group2 %in% "3", "T3", ifelse(tumor.stage.utest$group2 %in% "4", "T4", tumor.stage.utest$group2))))
    return(tumor.stage.utest)
  ...
  if(tnm.category == "metastasis"){
    # We create an empty dataframe to store the U-test data
    metastasis.utest <- data.frame(.y. = character(),
                        group1 = character(),
                        group2 = character(),
                        n1 = numeric(),
                        n2 = numeric(),
                        p = numeric())
    for(i in cancer.project){
    metastasis.df <- subset(all.tnm.df, project == i)
    # Filtering the dataframe to only have the metastasis data we want included in the analysis
    metastasis.df <- metastasis.df %>% select(!c(ajcc_pathologic_t, ajcc_pathologic_n, ajcc_clinical_m)) %>% filter(!(ajcc_pathologic_m %in% "MX") & sample_type %in% c("Metastatic", "Primary Blood Derived Cancer", "Primary Tumor", "Recurrent Tumor", "Additional - New Primary", "Primary Blood Derived Cancer - Peripheral Blood")) %>% drop_na(ajcc_pathologic_m)
    ...
    utest.results <- cinmetrics.metastasis.df %>% wilcox_test(cinmetrics ~ metastasis, paired = FALSE, p.adjust.method = "none")
    utest.results$project <- i
    utest.results$cin <- j
    metastasis.utest <- rbind(metastasis.utest, utest.results)
    }
    }
    metastasis.utest <- metastasis.utest %>% select(cin, project, group1, group2, n1, n2, statistic, p)
    metastasis.utest$group1 <- ifelse(metastasis.utest$group1 %in% "1", "M0", ifelse(metastasis.utest$group1 %in% "2", "M1", metastasis.utest$group1))
    metastasis.utest$group2 <- ifelse(metastasis.utest$group2 %in% "1", "M0", ifelse(metastasis.utest$group2 %in% "2", "M1", metastasis.utest$group1))
    return(metastasis.utest)
  }
...
#tumor.stage.utest <- get_utest_data(all.tnm.df, cancer.project, tnm.category = "tumor.stage", metrics)

metastasis.utest <- get_utest_data(all.tnm.df, cancer.project, tnm.category = "metastasis", metrics)

node.utest <- get_utest_data(all.tnm.df, cancer.project, tnm.category = "node", metrics)
