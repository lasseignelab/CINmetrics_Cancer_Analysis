...
get_utest_graphs <- function(tnm.category, cancer.project){
  ...
  if(tnm.category == "node"){
    ...
    filtered.node.utest <- filtered.node.utest %>% mutate(p.combined = coalesce(p.adj, p))
    log.node.utest <- filtered.node.utest %>% select(p.combined) %>% sapply(., neglog10)
    ...
    # Tumor Stage Bar Plot
    node.barplot <- ggarrange(base.segments.barplot,
                                     break.points.barplot,
                                     tai.barplot,
                                     fga.barplot,
                                     cna.barplot,
