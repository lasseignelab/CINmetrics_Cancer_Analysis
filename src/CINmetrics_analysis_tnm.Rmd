...
get_utest_data <- function(all.tnm.df, cancer.project, tnm.category, metrics){
  ...
  if(tnm.category == "metastasis"){
    # We create an empty data frames to store the U-test data
    metastasis.utest <- data.frame(.y. = character(),
                        ...
                        project = character(), 
                        cin = character())
    acc.metastasis.utest <- data.frame(.y. = character(),
                        group1 = character(),
                        group2 = character(),
                        n1 = numeric(),
                        n2 = numeric(),
    ...
    # Subset to have the U-test perform on each cancer
    metastasis.df <- subset(all.tnm.df, project == cancer.project[i])
    # Filtering the data frame to only have the metastasis data we want included in the analysis
   ...
   acc.metastasis <- factor(acc.metastasis.df[,"ajcc_clinical_m"], levels = c("M0", "M1"))
    ...
    acc.metastasis.utest <- rbind(acc.metastasis.utest, acc.utest.results)
    }
     }
    }
    # Cleaning the data frame to have M0 and M1 as group names
   ...
   acc.metastasis.utest <- acc.metastasis.utest %>% select(cin, project, group1, group2, n1, n2, statistic, p)
   acc.metastasis.utest$group1 <- ifelse(acc.metastasis.utest$group1 %in% "1", "M0", ifelse(acc.metastasis.utest$group1 %in% "2", "M1", acc.metastasis.utest$group1))
   acc.metastasis.utest$group2 <- ifelse(acc.metastasis.utest$group2 %in% "1", "M0", ifelse(acc.metastasis.utest$group2 %in% "2", "M1", acc.metastasis.utest$group1))
   metastasis.utest <- rbind(metastasis.utest, acc.metastasis.utest)
